...
interface Props {
  ...
  description: string;
  badge?: string; // 👈 新增：可选的标签参数
}

export default function WebMConverter({ h1Title, description, badge }: Props) {
      ...
      <div className="flex-grow flex items-center justify-center p-4 md:p-8">
        <div className="w-full max-w-xl bg-slate-900/60 backdrop-blur-md border border-slate-700/50 p-6 md:p-8 rounded-2xl shadow-2xl shadow-black/50">
          <div className="text-center mb-6 md:mb-8">
            ...
            {/* 🔥 这里是新增的 Badge 渲染逻辑 🔥 */}
            {badge && (
              <div className="inline-flex items-center gap-1.5 px-3 py-1 mb-4 rounded-full border border-blue-500/30 bg-blue-500/10 text-blue-400 text-[10px] md:text-xs font-mono font-bold uppercase tracking-wider animate-pulse">
                <span>✨</span>
                {badge}
              </div>
            )}
